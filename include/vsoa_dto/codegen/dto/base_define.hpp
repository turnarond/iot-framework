...
/**
 * Codegen macro to be used in classes extending &id:vsoa::data::mapping::type::Object; to generate required fields/methods/constructors for DTO object.
 * @param TYPE_NAME - name of the DTO class.
 * @param TYPE_EXTEND - name of the parent DTO class. If DTO extends &id:vsoa::data::mapping::type::Object; TYPE_EXETENDS should be `Object`.
 */
#define DTO_INIT(TYPE_NAME, TYPE_EXTEND) \
  template<class __Z__T__PARAM> \
  friend class vsoa::data::mapping::type::__class::Object; \
public: \
  typedef TYPE_NAME Z__CLASS; \
  typedef TYPE_EXTEND Z__CLASS_EXTENDED; \
  typedef vsoa::data::mapping::type::DTOWrapper<Z__CLASS> Wrapper; \
private: \
\
  static const vsoa::Type* getParentType() { \
    return vsoa::Object<Z__CLASS_EXTENDED>::Class::getType(); \
  } \
\
...
\
  static vsoa::data::mapping::type::BaseObject::Properties* Z__CLASS_GET_FIELDS_MAP(){ \
    static vsoa::data::mapping::type::BaseObject::Properties map = vsoa::data::mapping::type::BaseObject::Properties(); \
    return &map; \
  } \
\
public: \
...
#define VSOA_MACRO_DTO_FIELD_1(TYPE, NAME) \
\
...
\
static vsoa::data::mapping::type::BaseObject::Property* Z__PROPERTY_SINGLETON_##NAME() { \
  static vsoa::data::mapping::type::BaseObject::Property* property = \
      new vsoa::data::mapping::type::BaseObject::Property(Z__PROPERTY_OFFSET_##NAME(), \
                                                     #NAME, \
                                                     TYPE::Class::getType()); \
  return property; \
} \
...
#define VSOA_MACRO_DTO_FIELD_2(TYPE, NAME, QUALIFIER) \
\
static v_int64 Z__PROPERTY_OFFSET_##NAME() { \
  char buffer[sizeof(Z__CLASS)]; \
  auto obj = static_cast<Z__CLASS*>(reinterpret_cast<void*>(buffer)); \
...
VSOA_MACRO_EXPAND(VSOA_MACRO_MACRO_SELECTOR(VSOA_MACRO_DTO_FIELD_, (__VA_ARGS__)) (TYPE, __VA_ARGS__))
...
\
static void Z__PROPERTY_ADD_INFO_##NAME(vsoa::data::mapping::type::BaseObject::Property::Info* info)


#define DTO_FIELD_TYPE_SELECTOR(NAME) \
\
class Z__PROPERTY_TYPE_SELECTOR_##NAME : public vsoa::BaseObject::Property::FieldTypeSelector<Z__CLASS> { \
public: \
  const vsoa::Type* selectFieldType(Z__CLASS* self) override { \
...
\
const vsoa::Type* Z__PROPERTY_TYPE_SELECTOR_METHOD_##NAME()

// FOR EACH

#define VSOA_MACRO_DTO_HC_EQ_PARAM_HC(INDEX, COUNT, X) \
result = ((result << 5) - result) + std::hash<decltype(X)>{}(X);

#define VSOA_MACRO_DTO_HC_EQ_PARAM_EQ(INDEX, COUNT, X) \
&& X == other.X
...
v_uint64 hashCode() const { \
  ...
  VSOA_MACRO_FOREACH(VSOA_MACRO_DTO_HC_EQ_PARAM_HC, __VA_ARGS__) \
  return result; \
} \
\
bool operator==(const Z__CLASS& other) const { \
  ...
  VSOA_MACRO_FOREACH(VSOA_MACRO_DTO_HC_EQ_PARAM_EQ, __VA_ARGS__) \
  ; \
} \
\
bool operator!=(const Z__CLASS& other) const { \
