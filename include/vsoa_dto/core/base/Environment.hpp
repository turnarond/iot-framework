 ...
 *
 * Copyright  SylixOS
 *                         Benedikt-Alexander Mokroß <vsoa@bamkrs.de>
 *
...
#ifndef vsoa_base_Environment_hpp
#define vsoa_base_Environment_hpp
 ...
 * All calls to `VSOA_DISABLE_LOGV`, `VSOA_DISABLE_LOGD`, `VSOA_DISABLE_LOGI`,
 * `VSOA_DISABLE_LOGW`, `VSOA_DISABLE_LOGE` will come here.
 */
class Logger {
public:
  /**
...
#ifndef VSOA_COMPAT_BUILD_NO_THREAD_LOCAL
...
public:
   ...
   * *0 - if built with `-DVSOA_COMPAT_BUILD_NO_THREAD_LOCAL` flag*
   */
   ...
   * Print values for: <br>
   * - `VSOA_DISABLE_ENV_OBJECT_COUNTERS`<br>
   * - `VSOA_THREAD_HARDWARE_CONCURRENCY`<br>
   */
...
/**
 * Default vsoa assert method.
 * @param FMT - the format string used for the expression
 * @param EXP - expression that must be `true`.
 */
#define VSOA_ASSERT_FMT(FMT, EXP) \
if(!(EXP)) { \
  VSOA_LOGE("\033[1mASSERT\033[0m[\033[1;31mFAILED\033[0m]", FMT, #EXP) \
  exit(EXIT_FAILURE); \
}
 ...
 * @param EXP - expression that must be `true`.
 */
#define VSOA_ASSERT(EXP) \
if(!(EXP)) { \
  VSOA_LOGE("\033[1mASSERT\033[0m[\033[1;31mFAILED\033[0m]", #EXP) \
  exit(EXIT_FAILURE); \
}
 ...
 */
#define VSOA_DECLARE_LOG_CATEGORY(NAME) \
  static vsoa::base::LogCategory NAME;
 ...
 */
#define VSOA_LOG_CATEGORY(NAME, TAG, ENABLED) \
  vsoa::base::LogCategory NAME = vsoa::base::LogCategory(TAG, ENABLED);


#ifndef VSOA_DISABLE_LOGV

  /**
   * Log message with &l:Logger::PRIORITY_V; <br>
   * *To disable this log compile vsoa with `#define VSOA_DISABLE_LOGV`*
   * @param TAG - message tag.
   * @param ...(1) - message.
   * @param ... - optional format parameter.
   */
  #define VSOA_LOGV(TAG, ...) \
  vsoa::base::Environment::logFormatted(vsoa::base::Logger::PRIORITY_V, TAG, __VA_ARGS__);

#else
  #define VSOA_LOGV(TAG, ...)
#endif
  
#ifndef VSOA_DISABLE_LOGD

  /**
   * Log message with &l:Logger::PRIORITY_D; <br>
   * *To disable this log compile vsoa with `#define VSOA_DISABLE_LOGD`*
   * @param TAG - message tag.
   * @param ...(1) - message.
   * @param ... - optional format parameter.
   */
  #define VSOA_LOGD(TAG, ...) \
  vsoa::base::Environment::logFormatted(vsoa::base::Logger::PRIORITY_D, TAG, __VA_ARGS__);

#else
  #define VSOA_LOGD(TAG, ...)
#endif

#ifndef VSOA_DISABLE_LOGI

  /**
   * Log message with &l:Logger::PRIORITY_I; <br>
   * *To disable this log compile vsoa with `#define VSOA_DISABLE_LOGI`*
   * @param TAG - message tag.
   * @param ...(1) - message.
   * @param ... - optional format parameter.
   */
  #define VSOA_LOGI(TAG, ...) \
  vsoa::base::Environment::logFormatted(vsoa::base::Logger::PRIORITY_I, TAG, __VA_ARGS__);

#else
  #define VSOA_LOGI(TAG, ...)
#endif

#ifndef VSOA_DISABLE_LOGW

  /**
   * Log message with &l:Logger::PRIORITY_W; <br>
   * *To disable this log compile vsoa with `#define VSOA_DISABLE_LOGW`*
   * @param TAG - message tag.
   * @param ...(1) - message.
   * @param ... - optional format parameter.
   */
  #define VSOA_LOGW(TAG, ...) \
  vsoa::base::Environment::logFormatted(vsoa::base::Logger::PRIORITY_W, TAG, __VA_ARGS__);

#else
  #define VSOA_LOGW(TAG, ...)
#endif

#ifndef VSOA_DISABLE_LOGE

  /**
   * Log message with &l:Logger::PRIORITY_E; <br>
   * *To disable this log compile vsoa with `#define VSOA_DISABLE_LOGE`*
   * @param TAG - message tag.
   * @param ...(1) - message.
   * @param ... - optional format parameter.
   */
  #define VSOA_LOGE(TAG, ...) \
  vsoa::base::Environment::logFormatted(vsoa::base::Logger::PRIORITY_E, TAG, __VA_ARGS__);
...
#endif /* vsoa_base_Environment_hpp */
