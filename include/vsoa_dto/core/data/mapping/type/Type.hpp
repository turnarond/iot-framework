...
#ifndef vsoa_data_type_Type_hpp
#define vsoa_data_type_Type_hpp
...
public:
  ...
  struct Info {
    ...
    /**
     * polymorphicDispatcher extends &id:vsoa::data::mapping::type::__class::Collection::PolymorphicDispatcher;.
     */
     bool isCollection = false;

    /**
     * polymorphicDispatcher extends &id:vsoa::data::mapping::type::__class::Map::PolymorphicDispatcher;.
     */
     bool isMap = false;
  };
...
Wrapper ObjectWrapper<T, Clazz>::cast() const {
  if(!Wrapper::Class::getType()->extends(m_valueType)) {
    if(Wrapper::Class::getType() != __class::Void::getType() && m_valueType != __class::Void::getType()) {
      throw std::runtime_error("[vsoa::data::mapping::type::ObjectWrapper::cast()]: Error. Invalid cast "
...
void ObjectWrapper<T, Clazz>::checkType(const Type* _this, const Type* other) {
  if(!_this->extends(other)) {
    throw std::runtime_error("[vsoa::data::mapping::type::ObjectWrapper::checkType()]: Error. "
                             "Type mismatch: stored '" + std::string(_this->classId.name) + "' vs "
                             "assigned '" + std::string(other->classId.name) + "'.");
  }
}

#define VSOA_DEFINE_OBJECT_WRAPPER_DEFAULTS(WRAPPER_NAME, OBJECT_TYPE, OBJECT_CLASS) \
public: \
...
template<>
struct hash<vsoa::data::mapping::type::ClassId> {

  typedef vsoa::data::mapping::type::ClassId argument_type;
...
template<>
struct hash<vsoa::data::mapping::type::Void> {

  typedef vsoa::data::mapping::type::Void argument_type;
  typedef v_uint64 result_type;
