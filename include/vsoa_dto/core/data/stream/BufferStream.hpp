...
#ifndef vsoa_data_stream_BufferStream_hpp
#define vsoa_data_stream_BufferStream_hpp
...
public:
  ...
  /**
   * Copy data to &id:vsoa::String;.
   * @return
   */
  vsoa::String toString();

  /**
   * Create &id:vsoa::String; from part of buffer.
   * @param pos - starting position in buffer.
   * @param count - size of bytes to write to substring.
   * @return - &id:vsoa::String;
   */
  vsoa::String getSubstring(v_buff_size pos, v_buff_size count);
   ...
   */
  vsoa::v_io_size flushToStream(OutputStream* stream);
   ...
   */
  static vsoa::async::CoroutineStarter flushToStreamAsync(const std::shared_ptr<BufferOutputStream>& _this, const std::shared_ptr<OutputStream>& stream);
...
public:
   ...
   * @param captureData - capture auxiliary data to not get deleted until it's done with the stream.
   */
  BufferInputStream(const vsoa::String& data, const std::shared_ptr<void>& captureData = nullptr);
...
#endif // vsoa_data_stream_BufferStream_hpp
